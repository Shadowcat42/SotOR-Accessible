use crate::{
    ...
    ui::{
        styles::{
            set_checkbox_styles, set_combobox_styles, set_selectable_styles, set_spin_styles,
            BLACK, GREEN, GREEN_DARK, RED, WHITE,
        },
        widgets::{accessible_name, color_text, on_hover_text_side, Icon, UiExt},
        UiRef,
    },
...
impl<'a> CharAbilities<'a> {
    ...
    fn feats(&mut self, ui: UiRef) {
        ...
        Self::ability_list("feat", ui, list, &self.data.feats);
    }
            ...
            for (idx, name, description, _) in named_list {
                ...
                r.widget_info(|| {
                    WidgetInfo::labeled(
                        ...
                        format!(
                            "Remove {kind} {}",
                            accessible_name(galley.text(), description)
                        ),
                    )
                });
    ...
    fn selection<I: Eq + Hash + Copy, E: Data<I> + DataDescr>(
        ...
        let mut added = false;
        let response = ComboBox::from_id_source(id).width(240.).show_ui(ui, |ui| {
        ...
        response
            .response
            .widget_info(|| WidgetInfo::labeled(WidgetType::ComboBox, label));
    ...
    fn class(ui: UiRef, class: &mut Class, data: &GameDataMapped, mut remove: impl FnMut()) {
        ...
        Frame::default()
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    if ui
                        .s_icon_button(Icon::Remove, &format!("Remove class {name}"))
                        .clicked()
                    {
                        remove();
                    }
                ...
                Grid::new(format!("ec_class_{id}"))
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.s_checkbox(&mut checked, "Show all powers");
                            if checked != show_all {
                ...
                ui.s_empty();
                Self::ability_list("power", ui, list.as_mut().unwrap(), &data.powers);
            });
    }
}
