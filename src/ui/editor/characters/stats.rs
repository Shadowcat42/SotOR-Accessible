...
use egui::{ComboBox, DragValue, Grid, WidgetInfo, WidgetType};
...
impl<'a> CharStats<'a> {
    ...
    pub fn show(&mut self, ui: UiRef) {
        ...
        Self::grid("ec_general", ui, |ui| {
            ...
            ui.add_enabled_ui(false, |ui| {
                ui.s_spin(&mut self.char.hp_max, 0..=9999, true, "Maximum HP")
            });
            ui.end_row();
            ...
            ui.horizontal(|ui| {
                ...
                ui.s_checkbox(&mut self.char.min_1_hp, "Minimum 1 HP");
            });
            ui.end_row();
            ...
            ui.s_spin(&mut self.char.good_evil, 0..=100, false, "Alignment");
            ui.end_row();

            ui.label(color_text("XP: ", GREEN));
            ui.s_spin(&mut self.char.experience, 0..=9_999_999, true, "Experience");
            ui.end_row();
            ...
            if let Some(list) = &mut self.party_table.influence {
                if let Some(inf) = list.get_mut(self.char.idx) {
                    ...
                    ui.s_spin(inf, 0..=100, false, "Influence");
                    ui.end_row();
                }
            }
        });
        ...
        Self::grid("ec_skills", ui, |ui| {
            ...
            for (name, value) in labels.into_iter().zip(&mut self.char.skills) {
                ...
                response.widget_info(|| WidgetInfo {
                    label: Some(name.trim_end_matches([' ', ':']).to_owned()),
    ...
    fn appearance(&mut self, ui: UiRef) {
        ...
        Self::grid("ec_appearance", ui, |ui| {
            ...
            ui.end_row();
            let response = ComboBox::from_id_source("ec_gender")
                .width(200.)
            ...
            response.response.widget_info(|| {
                WidgetInfo::labeled(
                    WidgetType::ComboBox,
                    format!("Gender: {}", self.char.gender.to_str()),
                )
            });
            ui.end_row();
    ...
    fn appearance_selection(
        ...
        let label = match id {
            "ec_portrait" => "Portrait",
            "ec_appearance" => "Appearance",
            "ec_soundset" => "Soundset",
            _ => "Selection",
        };
        response
            .response
            .widget_info(|| WidgetInfo::labeled(WidgetType::ComboBox, format!("{label}: {name}")));
    }
}
