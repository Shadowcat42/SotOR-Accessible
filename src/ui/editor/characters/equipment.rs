use crate::{
    ...
    ui::{
        ...
        widgets::{accessible_name, color_text, on_hover_text_side, UiExt},
        UiRef,
    },
    util::{ColumnCounter, ContextExt},
};
use core::{Data, DataDescr, GameDataMapped, ItemSlot, UsableBy, WeaponType};
use egui::{ComboBox, Grid, Id, Response, WidgetInfo, WidgetType};
...
impl<'a> CharEquipment<'a> {
    ...
    fn header(&mut self, ui: UiRef) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.s_checkbox(&mut self.show_all, "Show all equipment templates");
        });
        ...
        ui.add_enabled_ui(!slots.is_empty(), |ui| {
            ...
            let r = ComboBox::from_id_source(id)
                ...
                .show_ui(ui, |ui| {
                    ...
                    for template in &self.data.inner.items {
                        ...
                        let selected_now = selected == Some(template.id.as_str());
                        r.widget_info(|| {
                            WidgetInfo::selected(
                                WidgetType::SelectableLabel,
                                selected_now,
                                accessible_name(template.get_name(), template.get_description()),
                            )
                        });
            ...
            r.response.widget_info(|| {
                WidgetInfo::labeled(
                    ...
                    format!(
                        "{}: {}",
                        slot_name.trim_end_matches([' ', ':']),
                        current.as_ref().map(Item::get_name).unwrap_or("None")
                    ),
                )
            });
