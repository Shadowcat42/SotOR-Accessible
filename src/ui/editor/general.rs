use crate::{
    ...
    ui::{
        styles::{set_checkbox_styles, set_spin_styles, set_striped_styles, BLUE, GREEN, WHITE},
...
impl<'a> Editor<'a> {
    ...
    fn main_table(&mut self, ui: UiRef) {
        ui.label("Save name: ");
        ui.s_text_edit(&mut self.nfo.save_name, 200., "Save name");
        ui.end_row();
        ...
        ui.horizontal(|ui| {
            ...
            ui.s_checkbox(&mut self.nfo.cheat_used, "Cheats used");
        });
        ui.end_row();
    ...
    fn member_row(
        ...
        if let Some((idx, _)) = in_party_idx {
            let btn = ui.s_icon_button(
                Icon::Leave,
                &format!("Remove {} from current party", char.get_name()),
            );
            ...
            let btn = ui.add_enabled(
                ...
                IconButton::new(Icon::Plus)
                    .hint(&format!("Add {} to current party", char.get_name())),
            );
        ...
        ui.s_checkbox(
            &mut member.available,
            &format!("{} available", char.get_name()),
        );
        ui.s_checkbox(
            &mut member.selectable,
            &format!("{} selectable", char.get_name()),
        );
    }
}
